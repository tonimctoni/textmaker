...
int main()
{
    ...
    const char *asoiaf_filename="../gutenberg/gutenberg.txt";
    static constexpr size_t output_length=2000;
    static constexpr size_t output_quantity=5;

    static constexpr size_t allowed_char_amount=94;
    ...
    const string index_to_char=" $(,048<DHLPTXdhlptx|#\'+/37;?CGKOSW[_cgkosw{\n\"&*.26:>BFJNRVZbfjnrvz~\t\r!%)-159=AEIMQUY]aeimquy}";
    ...
    size_t iteration=0;
    {
        ...
        if(in.good())
        {
            ...
            assert(save_counter!=0);
            {
                ...
                ifstream in(cbuffer, std::ios::binary);
                assert(in.good());
            ...
            learning_rate=0.01*pow(0.8912509381337456, (iteration/1000));// gets divided by 10 every 20k stepss
        }
        else
        {
    ...
    auto first_time=chrono::steady_clock::now();
    ...
    for(;;iteration++)
    {
        ...
        if(chrono::steady_clock::now()-last_time>time_between_saves)
        {
