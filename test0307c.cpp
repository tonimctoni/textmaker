...
int main()
{
    ...
    perceptronblock->reserve_time_steps(max_training_chars);
    ...
    size_t iteration=0;
    {
        ...
        else
        {
            ...
            double error=1.0;
            for(size_t iteration=0;error>0.000001;iteration++)
            {
                X[0][last_input_index]=0.0;
                last_input_index=dst(gen);
                X[0][last_input_index]=1.0;

                perceptronblock->calc(X, 0);
                softmaxblock2->calc(perceptronblock->get_output(0), 0);

                softmaxblock2->set_first_delta_and_propagate_with_cross_enthropy(X, perceptronblock->get_delta_output(0), 0);
                // softmaxblock2->set_first_delta(X, 0);
                // softmaxblock2->propagate_delta(perceptronblock->get_delta_output(0), 0);
                perceptronblock->propagate_delta(0);

                perceptronblock->update_weights(X,0,0.2);
                softmaxblock2->update_weights(perceptronblock->get_output(0),0,0.2);
                ...
                if(iteration%10000==0)
                {
                    ...
                    for(size_t new_input_index=0;new_input_index<input_size;new_input_index++)
                    {
                        ...
                        {
                            double aux=softmaxblock2->get_output(0)[0][i]-X[0][i];
                            aux*=aux;
                            error+=aux;
                        }
                    }
    ...
    for(;;iteration++)
    {
        ...
        for(size_t i=asoiaf_length-1;;)
        {
            ...
            softmaxblock->set_first_delta_and_propagate_with_cross_enthropy(Y, lstmblock2->get_delta_output(i), i);
        ...
        for(size_t i=0;i<asoiaf_length;i++)
        {
            ...
            last_input_index=char_to_index[asoiaf_content[asoiaf_start+i-1]];
            X[0][last_input_index]=1.0;
            perceptronblock->update_weights(X, i, learning_rate);
            lstmblock1->update_weights(perceptronblock->get_output(i), i, learning_rate);
        ...
        if(time(nullptr)-last_time>secons_between_saves)
        {
            ...
            sprintf(cbuffer, output_filename, save_counter);
            {
                ...
                for(size_t ex=0;ex<10;ex++)
                {
                    ...
                    perceptronblock->set_time_steps(2000);
