...
public:
    ...
    TahnPerceptronBlock(size_t time_steps=0) noexcept:layer_states(time_steps),moment_weights(0.0),moment_bias(0.0),ms_weights(1.0), ms_bias(1.0), weight_gradient_accumulator(0.0), bias_gradient_accumulator(0.0)
    {
...
public:
    ...
    SoftmaxBlock(size_t time_steps=0) noexcept:layer_states(time_steps),moment_weights(0.0),moment_bias(0.0),ms_weights(1.0), ms_bias(1.0), weight_gradient_accumulator(0.0), bias_gradient_accumulator(0.0)
    {
        weights.randomize_for_nn(input_size+1);
        bias.randomize_for_nn(input_size+1);
    }
...
// inline void update_weights(const double learning_rate, const double ms_rate) noexcept
// {
//     weights.add_a_mul_rate_div_sqrt_b(moment_weights, learning_rate, ms_weights);
//     bias.add_a_mul_rate_div_sqrt_b(moment_bias, learning_rate, ms_bias);

//     ms_weights.mul(1-ms_rate);
//     ms_bias.mul(1-ms_rate);
//     ms_weights.add_factor_mul_a_mul_a(ms_rate, moment_weights);
//     ms_bias.add_factor_mul_a_mul_a(ms_rate, moment_bias);
// }