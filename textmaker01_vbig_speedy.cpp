...
int main()
{
    ...
    using Block01=SpeedyTahnPerceptronBlock<input_size,reduced_input_size>;
    using Block02=SpeedyLSTMBlock<reduced_input_size, first_mem_cell_size>;
    using Block03=SpeedyLSTMBlock<first_mem_cell_size, second_mem_cell_size>;
    using Block04=SpeedyLSTMBlock<second_mem_cell_size, third_mem_cell_size>;
    using Block05=SpeedySoftmaxBlock<third_mem_cell_size, output_mem_size>;
    ...
    size_t iteration=0;
    {
        ...
        else
        {
            ...
            using BlockPre=SpeedySoftmaxBlock<reduced_input_size,input_size>;
            ...
            for(size_t iteration=0;error>0.00000001;iteration++)
            {
                ...
                perceptronblock->update_weights_momentum_ms(learning_rate, decay);
                softmaxblock2->update_weights_momentum_ms(learning_rate, decay);
    ...
    for(;;iteration++)
    {
        if(iteration%1000==0)
        {
            learning_rate=0.01*pow(0.954992586021436, (iteration/1000));// gets divided by 10 every 50k steps
        ...
        perceptronblock->update_weights_momentum_ms(learning_rate, decay);
        lstmblock1->update_weights_momentum_ms(learning_rate, decay);
        lstmblock2->update_weights_momentum_ms(learning_rate, decay);
        lstmblock3->update_weights_momentum_ms(learning_rate, decay);
        softmaxblock->update_weights_momentum_ms(learning_rate, decay);
        ...
        if(time(nullptr)-last_time>secons_between_saves)
        {
            ...
            sprintf(cbuffer, output_filename, save_counter);
            {
                ...
                out << "Optimizer: " << "nag + rmsprop" << endl;
