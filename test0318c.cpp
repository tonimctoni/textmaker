...
int main()
{
    ...
    static constexpr double decay1=0.999;
    static constexpr double decay2=0.9;
    static constexpr size_t batch_size=5;


    using Block01=AdamTahnPerceptronBlock<input_size,reduced_input_size>;
    using Block02=AdamRELULSTMBlock<reduced_input_size, first_mem_cell_size>;
    using Block03=AdamRELULSTMBlock<first_mem_cell_size, second_mem_cell_size>;
    using Block04=AdamRELULSTMBlock<second_mem_cell_size, third_mem_cell_size>;
    using Block05=AdamSoftmaxBlock<third_mem_cell_size, output_mem_size>;
    ...
    size_t iteration=0;
    {
        ...
        else
        {
            ...
            using BlockPre=AdamSoftmaxBlock<reduced_input_size,input_size>;
            ...
            for(size_t iteration=0;error>0.00000001;iteration++)
            {
                ...
                perceptronblock->update_weights_adam(learning_rate, decay1, decay2);
                softmaxblock2->update_weights_adam(learning_rate, decay1, decay2);
    ...
    for(;;iteration++)
    {
        ...
        perceptronblock->update_weights_adam(learning_rate, decay1, decay2);
        lstmblock1->update_weights_adam(learning_rate, decay1, decay2);
        lstmblock2->update_weights_adam(learning_rate, decay1, decay2);
        lstmblock3->update_weights_adam(learning_rate, decay1, decay2);
        softmaxblock->update_weights_adam(learning_rate, decay1, decay2);
